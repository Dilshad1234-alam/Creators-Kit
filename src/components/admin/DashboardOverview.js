...
export default function DashboardOverview() {
    ...
    return (
      <div className="space-y-8 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-zinc-900/40 border border-zinc-800/40 rounded-3xl p-8 h-40">
              <div className="h-4 bg-zinc-800/60 rounded w-1/3 mb-4"></div>
              <div className="h-10 bg-zinc-800/60 rounded w-1/2"></div>
            </div>
          ))}
        </div>
        {/* Skeleton Table */}
        <div className="bg-zinc-900/40 border border-zinc-800/40 rounded-3xl p-8 h-96">
          <div className="h-6 bg-zinc-800/60 rounded w-1/4 mb-8"></div>
          <div className="space-y-4">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="h-12 bg-zinc-800/40 rounded w-full"></div>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-8">
      {/* Stat Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="relative group bg-zinc-900/80 border border-zinc-800/80 shadow-2xl rounded-3xl p-8 backdrop-blur-xl hover:border-[#FF3B14]/50 transition-all duration-300 overflow-hidden transform hover:-translate-y-1">
          ...
          <p className="text-5xl font-black text-white mt-2">{stats.products}</p>
        </div>
        ...
        <div className="relative group bg-zinc-900/80 border border-zinc-800/80 shadow-2xl rounded-3xl p-8 backdrop-blur-xl hover:border-[#FF3B14]/50 transition-all duration-300 overflow-hidden transform hover:-translate-y-1">
          ...
          <p className="text-5xl font-black text-white mt-2">{stats.orders}</p>
        </div>

        <div className="relative group bg-zinc-900/80 border border-zinc-800/80 shadow-2xl rounded-3xl p-8 backdrop-blur-xl hover:border-[#FF3B14]/50 transition-all duration-300 overflow-hidden transform hover:-translate-y-1">
          <div className="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 bg-[#FF3B14] rounded-full blur-[80px] opacity-20 group-hover:opacity-40 transition-opacity pointer-events-none"></div>
          <div className="flex justify-between items-start">
            <h3 className="text-zinc-400 font-bold mb-2 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#FF3B14] shadow-[0_0_8px_rgba(255,59,20,0.8)] animate-pulse"></span>
              Total Revenue
            </h3>
            <span className="bg-[#FF3B14]/10 text-[#FF3B14] text-xs font-bold px-2 py-1 rounded-full border border-[#FF3B14]/20">Verified</span>
          </div>
          <p className="text-5xl font-black text-[#FF3B14] mt-2">₹{stats.revenue.toLocaleString()}</p>
        </div>
      </div>

      {/* Recent Activity Table */}
      <div className="bg-zinc-900/80 border border-zinc-800/80 shadow-2xl rounded-3xl p-8 backdrop-blur-xl">
        <h2 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
          Recent Orders & Store Activity
          <span className="bg-zinc-800 text-zinc-400 text-xs font-bold px-3 py-1 rounded-full">Last 5 Orders</span>
        </h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-zinc-800/30">
              {recentOrders.map((o) => (
                <tr key={o._id} className="hover:bg-zinc-800/20 transition-colors group">
                  <td className="py-6 pr-4 align-top">
                    <div className="font-bold text-zinc-200">{o.customerName}</div>
                    <div className="text-xs text-zinc-500 mt-1 font-mono">ID: {o._id}</div>
                  </td>
                  <td className="py-6 px-4 align-top">
                    <div className="space-y-2">
                      {o.items?.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-zinc-950/40 p-2 rounded-lg border border-zinc-800/30">
                          <span className="text-sm text-zinc-300 truncate max-w-[150px]">{item.name}</span>
                          <span className="text-xs font-bold text-zinc-500 bg-zinc-900 px-2 py-1 rounded">x{item.quantity}</span>
                        </div>
                      ))}
                    </div>
                  </td>
                  <td className="py-6 px-4 align-top">
                    <div className="font-bold text-[#FF3B14] text-lg">₹{o.totalAmount?.toLocaleString()}</div>
                  </td>
              ...
              {recentOrders.length === 0 && (
                <tr>
                  <td colSpan="4" className="py-10 text-center text-zinc-500">No recent activity.</td>
                </tr>
              )}
            </tbody>
          </table>
