...
export default function OrdersManager() {
  ...
  return (
    <div className="bg-background/80 border border-neutral-800/80 rounded-3xl p-8 backdrop-blur-2xl shadow-2xl relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-8 relative z-12">
        <div>
          <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-neutral-500 tracking-tight mb-1">Orders Management</h2>
          <p className="text-sm text-neutral-400">Track and manage your storefront orders.</p>
        </div>
        <div className="bg-neutral-900 border border-neutral-800 rounded-full px-4 py-2 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
          <span className="text-xs font-bold text-neutral-300">{orders.length} Total Orders</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto relative z-10">
        <table className="w-full text-left border-collapse min-w-[1200px]">
          <thead>
            <tr className="text-neutral-400 uppercase text-[10px] font-black tracking-widest border-b border-neutral-800/80">
            ...
            {orders.map((o) => (
              <tr key={o._id} className="hover:bg-white/[0.02] transition-colors group">
                <td className="py-6 pr-4 align-top">
                  <div className="font-bold text-neutral-100 text-sm mb-1">{o.orderId || `ORD-${o._id.substring(o._id.length - 6).toUpperCase()}`}</div>
                  <div className="text-[10px] font-mono text-neutral-500 mb-2 truncate max-w-[100px]">{o._id}</div>
                  <div className="text-xs text-neutral-400 flex items-center gap-1 mb-2">
                ...
                <td className="py-6 px-4 align-top">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-neutral-700 to-neutral-900 flex items-center justify-center text-xs font-bold text-neutral-100 border border-neutral-700 shadow-inner flex-shrink-0">
                      {getInitials(o.customerName)}
                    </div>
                    <div className="font-bold text-neutral-200 text-sm whitespace-nowrap">{o.customerName}</div>
                  </div>
                </td>

                {/* Email Column */}
                <td className="py-6 px-4 align-top">
                  <a href={`mailto:${o.customerEmail}`} className="text-sm text-neutral-300 hover:text-primary transition-colors inline-block">{o.customerEmail}</a>
                </td>

                {/* Address Column */}
                <td className="py-6 px-4 align-top">
                  {o.shippingAddress ? (
                    <div className="text-sm text-neutral-400 leading-relaxed max-w-[200px]">
                      <span className="text-neutral-200 block">{o.shippingAddress.street}</span>
                ...
                <td className="py-6 px-4 align-top">
                  <div className="space-y-1.5">
                    {o.items?.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center bg-neutral-900/40 p-2 rounded-md border border-neutral-800/30 group-hover:border-neutral-700/50 transition-colors">
                        <div className="flex items-center gap-2 overflow-hidden">
                          <div className="w-1 h-4 bg-neutral-700 rounded-full"></div>
                          <span className="text-xs text-neutral-300 truncate font-medium" title={item.name}>{item.name}</span>
                        </div>
                        <div className="flex items-center gap-2 pl-2">
                          <span className="text-[10px] text-neutral-500 bg-neutral-800 px-1.5 py-0.5 rounded">x{item.quantity}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <td className="py-6 px-4 align-top">
                  <div className="font-black text-neutral-100 text-lg tracking-tight mb-2">₹{o.totalAmount?.toLocaleString()}</div>
                ...
                <td className="py-6 px-4 align-top">
                  <div className={`relative inline-block border rounded-full overflow-hidden transition-all duration-300 ${getStatusColor(o.status)}`}>
                    ...
                    >
                      ...
                      <option value="Processing" className="bg-neutral-900 text-purple-500">Processing</option>
                      <option value="Shipped" className="bg-neutral-900 text-blue-500">Shipped</option>
                      <option value="Delivered" className="bg-neutral-900 text-emerald-500">Delivered</option>
                      <option value="Cancelled" className="bg-neutral-900 text-rose-500">Cancelled</option>
                    </select>
            ...
            {orders.length === 0 && (
              <tr>
                <td colSpan="7" className="py-16 text-center">
                  ...
                  <h3 className="text-neutral-300 font-bold mb-1">No orders yet</h3>
