...
export default function ProductPage() {
  ...
  const product = {
    ...
    images: [
      { type: 'light', src: '/kit 4.webp', bg: 'bg-zinc-900' },
      { type: 'mic', src: '/kit 2.webp', bg: 'bg-zinc-900' },
      { type: 'mic-case', src: '/kit 3.webp', bg: 'bg-zinc-900' },
      { type: 'tripod', src: '/kit 1.webp', bg: 'bg-zinc-900' },
      { type: 'screen', src: '/kit.webp', bg: 'bg-zinc-900' },
      { type: 'pendrive', src: '/kit 5.webp', bg: 'bg-zinc-900' },
    ],
  };
  ...
  const partDetails = [
    { title: 'Perfect Lighting', desc: '10-inch ring light with 3 color modes and 10 brightness levels.', img: '/kit 4.webp' },
    { title: 'Clear Audio', desc: 'Wireless plug-and-play mic with active noise cancellation.', img: '/kit 2.webp' },
    { title: 'Stable Shots', desc: 'Durable floor stand and flexible desk tripod included.', img: '/kit 1.webp' },
    { title: 'Clean Backgrounds', desc: 'Wrinkle-resistant green screen for easy chroma keying.', img: '/kit.webp' },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-zinc-950 text-zinc-100 font-sans w-full">
      <main className="flex-grow w-full">
        
        {/* Sticky Action Bar for Mobile (Hidden on Desktop) */}
        <div className="fixed bottom-0 left-0 w-full bg-zinc-950 border-t border-zinc-800 p-4 z-50 md:hidden flex gap-3 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
           ...
           <button onClick={handleBuyNow} className="flex-1 bg-[#FC1D00] text-white rounded-full font-bold shadow-lg hover:bg-[#E01900] transition-colors">
             Buy Now
           </button>
        </div>

        {/* 1. Main Product Overview Section */}
        <section className="w-full bg-zinc-950 pt-12 pb-24 border-b border-zinc-900 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-full h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-red-500/10 via-transparent to-transparent opacity-70 pointer-events-none"></div>
          ...
          <div className="w-full px-6 md:px-12 lg:px-24 mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 relative z-10">
            ...
            <div className="space-y-6">
              {/* Main Image Viewport */}
              <div className={`w-full aspect-square rounded-[3rem] flex items-center justify-center shadow-2xl shadow-zinc-950/50 border border-zinc-800 transition-all duration-700 relative overflow-hidden bg-zinc-900/90 p-6 hover:border-[#FC1D00] transform hover:scale-[1.02] ${product.images[currentImage].bg}`}>
                <Image
                ...
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`relative snap-center shrink-0 w-24 h-24 rounded-2xl flex items-center justify-center border-2 transition-all overflow-hidden bg-zinc-900/90 ${
                      currentImage === idx 
                        ? 'border-[#FC1D00] ring-4 ring-[#FC1D00]/20 scale-105 shadow-lg' 
                        : 'border-zinc-800 opacity-60 hover:opacity-100 hover:scale-105 hover:border-[#FC1D00]'
                    }`}
                  >
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="bg-zinc-900 p-8 rounded-3xl border border-zinc-800 mb-10 shadow-sm relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-[#FC1D00] text-white text-xs font-bold px-4 py-1 rounded-bl-xl tracking-wider">SAVE 55%</div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-12">
                <button
                  ...
                  className="hidden md:flex flex-1 py-5 px-8 bg-zinc-900 border border-zinc-700 text-zinc-100 rounded-full font-bold text-lg hover:border-[#FC1D00] hover:text-[#FC1D00] hover:bg-zinc-950 transition-colors items-center justify-center gap-3 shadow-sm"
                >
                ...
                <button
                  ...
                  className="hidden md:flex flex-1 py-5 px-8 bg-[#FC1D00] text-white rounded-full font-extrabold text-xl hover:bg-[#E01900] transition-all hover:scale-105 shadow-xl shadow-red-500/30 items-center justify-center gap-2"
                >
                  Buy Now
        ...
        <section className="w-full py-24 bg-zinc-950 border-t border-zinc-900 relative overflow-hidden">
                ...
                <ul className="space-y-4">
                  {includedItems.map((item, idx) => (
                    <li key={idx} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-zinc-800 transition-colors border border-transparent hover:border-zinc-700">
                      <div className="flex-shrink-0 w-12 h-12 bg-zinc-950 rounded-xl flex items-center justify-center text-xl border border-zinc-800">
                        {item.icon}
                      </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {partDetails.map((part, idx) => (
                  <div key={idx} className="bg-zinc-900/50 backdrop-blur-md p-8 rounded-3xl border border-zinc-800 hover:border-[#FC1D00]/50 transition-colors group overflow-hidden">
                    <div className="relative w-full aspect-square mb-6 rounded-2xl overflow-hidden bg-zinc-900/90 border border-zinc-800 p-4 shadow-2xl hover:border-[#FC1D00] transition duration-300 transform hover:scale-[1.02]">
        ...
        <section className="w-full py-24 bg-zinc-900">
          <div className="w-full px-6 md:px-12 lg:px-24 mx-auto text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
              <div className="bg-zinc-950 p-10 rounded-[3rem] border border-zinc-800 shadow-xl flex flex-col items-center group hover:-translate-y-2 transition-transform">
            ...
            <div className="bg-zinc-950 rounded-[3rem] p-10 md:p-16 border border-zinc-800 grid grid-cols-1 md:grid-cols-2 gap-12 items-center text-left relative overflow-hidden shadow-2xl">
               <div className="absolute top-1/2 right-0 w-96 h-96 bg-[#FC1D00]/20 rounded-full blur-[100px] -translate-y-1/2"></div>
               <div className="relative z-10">
                 ...
                 <ul className="space-y-4 font-bold">
                   <li className="flex items-center gap-3"><span className="text-[#FC1D00] text-2xl">💾</span> <span className="text-zinc-300">High-Speed USB Drive</span></li>
                   <li className="flex items-center gap-3"><span className="text-[#FC1D00] text-2xl">📑</span> <span className="text-zinc-300">Printed Project Guides</span></li>
                 </ul>
               </div>
               <div className="relative z-10 w-full aspect-square rounded-3xl overflow-hidden shadow-2xl border border-zinc-800 bg-zinc-900/90 p-4 hover:border-[#FC1D00] transition duration-300 transform hover:scale-[1.02]">
          ...
          <div className="w-full px-6 md:px-12 lg:px-24 mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
            ...
            <div>
              ...
              <div className="space-y-4">
                {faqs.map((faq, index) => (
                  <div key={index} className={`border ${openFaqIndex === index ? 'border-[#FC1D00]' : 'border-zinc-800'} rounded-[1.5rem] overflow-hidden bg-zinc-900 shadow-sm transition-colors duration-300`}>
                    <button
